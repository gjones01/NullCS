  ...
  for (let iter = 0; iter < ITERATIONS; iter++) {
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        ...
        u[idx] = omega * nextValue + (1 - omega) * u[idx];
      }
    }
  }
...
export default function MetallicPaint({
  ...
  const mouseAnimationRef = useRef(mouseAnimation);
  ...
  useEffect(() => {
    mouseAnimationRef.current = mouseAnimation;
  }, [mouseAnimation]);
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      const imgData = processImage(image);
