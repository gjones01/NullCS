...
- Production analysis endpoints require `X-NULLCS-KEY`, with the expected value supplied from the server-side `NULLCS_API_KEY` environment variable.
- Real secrets must live in a local or deployment `.env` / environment manager and must never be committed to Git.
